        ...
        function(done){
            Tutor.findOne({email: req.body.email}, function(err, tut){
                if(err) res.status(200).send(err);
                    ...
                    const tutorId = tut.tutorId;
                    done(null, tutorId);
                }
            })
        }, function(tutorId, done){
            Tutor.updateOne({email: req.body.email}, {deleteConfirmed: true}, function(err, tut){
                if(err) res.status(200).send(err);
                    ...
                    let site = (process.env.NODE_ENV === 'production') ? "https://uf-tutor-match.herokuapp.com" : "localhost:3000"
                    let link = site + '/tutors/' + 'delete/'+ tutorId;
                    let mailOptions = {
                        from: '"UF Tutor Match" <donotreply@uftutormatch.com>',
                        to: req.body.email,
...
exports.list = (req, res) => {
    Tutor.find({confirmed:true}, function(err, tutor) {
        if (err) {
            res.statusCode = 404;
        ...
        function(done){
            ...
            Tutor.findOne({email: req.body.email}, function(err, tut){
                if(err) res.status(200).send(err);
                    ...
                    newUpdate.tutorId = tut.tutorId;
                    done(null, newUpdate);
                }
            })

        }, function(newUpdate, done){
